...
class AppLoader {
    ...
    decode = decodeToml) {
        if (!(await fileExists(filepath))) {
            return this.abortOrReport(outputContent `Couldn't find the configuration file at ${outputToken.path(filepath)}`, '', filepath);
        }
        ...
        try {
            ...
            if (err.line && err.pos && err.col) {
                return this.abortOrReport(outputContent `Fix the following error in ${outputToken.path(filepath)}:\n${err.message}`, null, filepath);
            }
            else {
                throw err;
            }
    ...
    decode = decodeToml) {
        ...
        if (!parseResult.success) {
            ...
            return this.abortOrReport(outputContent `Fix a schema error in ${outputToken.path(filepath)}:\n${formattedError}`, fallbackOutput, filepath);
        }
    ...
    async loadUIExtensions(extensionDirectories) {
        ...
        const extensions = configPaths.map(async (configurationPath) => {
            ...
            if (!specification) {
                ...
                this.abortOrReport(outputContent `Unknown extension type ${outputToken.yellow(type)} in ${outputToken.path(configurationPath)}. ${isShopifolk ? shopifolkMessage : ''}`, undefined, configurationPath);
                return undefined;
            }
    ...
    async loadFunctions(extensionDirectories) {
        ...
        const allFunctions = configPaths.map(async (configurationPath) => {
            ...
            const specification = this.findSpecificationForType(type);
            if (!specification) {
                this.abortOrReport(outputContent `Unknown function type ${outputToken.yellow(type)} in ${outputToken.path(configurationPath)}`, undefined, configurationPath);
                return undefined;
            }
    ...
    async loadThemeExtensions(extensionDirectories) {
        ...
        const extensions = configPaths.map(async (configurationPath) => {
            ...
            if (!specification) {
                this.abortOrReport(outputContent `Unknown theme type ${outputToken.yellow('theme')} in ${outputToken.path(configurationPath)}`, undefined, configurationPath);
                return undefined;
            }
